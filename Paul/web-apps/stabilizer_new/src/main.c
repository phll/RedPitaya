...
static rp_app_params_t rp_main_params[PARAMS_NUM+1] = {
	...
	{ /* lock_off offset for pid output on target channel*/
	    ...
	    { /* pid_NN_ki - PID NN integral gain     Ki in [ADC] counts. */
	        "pid_11_ki",  0, 1, 0, -2147483647-1, 2147483647 },
	    ...
	    { /* pid_NN_ki - PID NN integral gain     Ki in [ADC] counts. */
	        "pid_12_ki",  0, 1, 0, -2147483647-1, 2147483647 },
		{ /* pid_NN_damping - PID NN damping factor in permil. */
	    ...
	    { /* pid_NN_ki - PID NN integral gain     Ki in [ADC] counts. */
	        "pid_21_ki",  0, 1, 0, -2147483647-1, 2147483647 },
		{ /* pid_NN_damping - PID NN damping factor in permil. */
	    ...
	    { /* pid_NN_ki - PID NN integral gain     Ki in [ADC] counts. */
	        "pid_22_ki",  0, 1, 0, -2147483647-1, 2147483647 },
		{ /* pid_NN_damping - PID NN damping factor in permil. */
		...
		{ /* pid_NN_ki - PID NN integral gain     Ki in [ADC] counts. */
		    "pid_121_ki",  0, 1, 0, -2147483647-1, 2147483647 },
		{ /* pid_NN_damping - PID NN damping factor in permil. */
    ...
    for(i = 0; i < len && p[i].name != NULL; i++) {
        ...
        if(rp_main_params[p_idx].value != p[i].value) {
            ...
            if(p_idx >= PARAMS_PID_PARAMS && p_idx<PARAMS_CMD_PARAMS){
                pid_params_change = 1;
								fprintf(stderr, "pid11 ki: %fd, %dd, %xh\n",rp_main_params[PID_11_KI].value,(int)rp_main_params[PID_11_KI].value,(int)rp_main_params[PID_11_KI].value);
						}
