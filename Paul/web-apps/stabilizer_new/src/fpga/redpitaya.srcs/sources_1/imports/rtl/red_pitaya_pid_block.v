/**
 * $Id: red_pitaya_pid_block.v 961 2014-01-21 11:40:39Z matej.oblak $
 *
 * @brief Red Pitaya PID controller.
 *
 * @Author Matej Oblak
 *
 * (c) Red Pitaya  http://www.redpitaya.com
 *
 * This part of code is written in Verilog hardware description language (HDL).
 * Please visit http://en.wikipedia.org/wiki/Verilog
 * for more details on the language used herein.
 */



/**
 * GENERAL DESCRIPTION:
 *
 * Proportional-integral-derivative (PID) controller.
 *
 *
 *        /---\         /---\      /-----------\
 *   IN --| - |----+--> | P | ---> | SUM & SAT | ---> OUT
 *        \---/    |    \---/      \-----------/
 *          ^      |                   ^  ^
 *          |      |    /---\          |  |
 *   set ----      +--> | I | ---------   |
 *   point         |    \---/             |
 *                 |                      |
 *                 |    /---\             |
 *                 ---> | D | ------------
 *                      \---/
 *
 *
 * Proportional-integral-derivative (PID) controller is made from three parts. 
 *
 * Error which is difference between set point and input signal is driven into
 * propotional, integral and derivative part. Each calculates its own value which
 * is then summed and saturated before given to output.
 *
 * Integral part has also separate input to reset integrator value to 0.
 * 
 */




module red_pitaya_pid_block #(
   parameter     PSR = 12         ,
   parameter     ISR = 18+13         ,
   parameter     DSR = 10          
)
(
   // data
   input                 clk_i           ,  // clock
   input                 rstn_i          ,  // reset - active low
   input      [ 14-1: 0] dat_i           ,  // input data
   output     [ 14-1: 0] dat_o           ,  // output data
   

   // settings
   input      [ 14-1: 0] set_sp_i        ,  // set point
   input      [ 14-1: 0] set_kp_i        ,  // Kp
   input      [ 32-1: 0] set_ki_i        ,  // Ki
   input      [ 14-1: 0] set_damping_i   ,  // damping
   input      [ 14-1: 0] set_kd_i        ,  // Kd
   input                 int_rst_i          // integrator reset
);




//---------------------------------------------------------------------------------
//  Set point error calculation

reg  [ 15-1: 0] error        ;

always @(posedge clk_i) begin
   if (rstn_i == 1'b0) begin
      error <= 15'h0 ;
   end
   else begin
      error <= $signed(set_sp_i) - $signed(dat_i) ;
   end
end






//---------------------------------------------------------------------------------
//  Proportional part

reg   [29-PSR-1: 0] kp_reg        ;
wire  [    29-1: 0] kp_mult       ;

always @(posedge clk_i) begin
   if (rstn_i == 1'b0) begin
      kp_reg  <= {29-PSR{1'b0}};
   end
   else begin
      kp_reg <= kp_mult[29-1:PSR] ;
   end
end

assign kp_mult = $signed(error) * $signed(set_kp_i);








//---------------------------------------------------------------------------------
//  Integrator

reg   [    15+32-1  : 0] ki_mult       ;  //biggest result of error * set_ki_i has 15+32 bit
wire  [    33+15-1: 0] int_sum       ;  //needs one more bit than int_reg to prevent overflow
reg   [    32+15-1: 0] int_reg       ;  /****
                                        **error is scaled by a factor € +/-[2^-(18+13), 2^-(18-(13+5))]u{0} = +/-[2^-31, 1]u{0}. 
                                        **Since scaling works by first scaling up the error and then scaling down the total sum in the end,
                                        **the total sum has to be large enough for the maximum scaled up error (15+32 bit) to hold
                                        *****/
//reg   [    42-1: 0] int_t         ;
//reg   [    16-1: 0] counter       ;         //125MHz clock cycle (8ns per cycle) -> 125MHz/2^(16-1) = 1907Hz damping frequency -> 363ms max half-life period
wire  [32+15-ISR-1: 0] int_shr       ;  //16 bits

always @(posedge clk_i) begin
   if (rstn_i == 1'b0) begin
      ki_mult  <= {15+32{1'b0}};
      int_reg  <= {32+15{1'b0}};
      //counter <= 16'b0;
   end
   else begin
      ki_mult <= $signed(error) * $signed(set_ki_i) ;
         
      if (int_rst_i)
        int_reg <= 32+15'h0; // reset
      else if (int_sum[33+15-1:33+15-2] == 2'b01) // positive saturation
        int_reg <= 32+15'h3FFFFFFFFFFF; // max positive
      else if (int_sum[33+15-1:33+15-2] == 2'b10) // negative saturation
        int_reg <= 32+15'h400000000000; // max negative
      else
        int_reg <= int_sum[32+15-1:0]; // use sum as it is
      ki_mult <= $signed(error) * $signed(set_ki_i) ;

      /*
      if (int_rst_i) begin
         int_reg <= 32'h0; // reset
         //counter <= 16'b0;
      end
      else begin
        
         if (int_sum[33-1:33-2] == 2'b01) // positive saturation
            int_t = {{10{0}},32'h7FFFFFFF};// {10 MSB, max positive}
         else if (int_sum[33-1:33-2] == 2'b10) // negative saturation
            int_t = {{10{1}},32'h80000000};// {10 MSB, max negative}
         else
            int_t = {{10{int_sum[32-1]}},int_sum[32-1:0]};
        counter = counter + 16'b1;
        if(counter[16-1] == 1'b1) begin
            int_t = $signed(int_t) * $unsigned(set_damping_i);
            int_t = int_t >>> 10;//"divide" by 1024
            counter = 16'b0;//reset counter
        end
        int_reg <= int_t[32-1:0];
      end*/
   end
end

assign int_sum = $signed(ki_mult) + $signed(int_reg) ;
assign int_shr = int_reg[32+15-1:ISR] ;   //divide by ISR (=18+13). In the end we multiplied 'error' by ki/ISR € +/-[2^-(18+13), 2^-(18-(13+5))]u{0} 






//---------------------------------------------------------------------------------
//  Derivative

wire  [    29-1: 0] kd_mult       ;
reg   [29-DSR-1: 0] kd_reg        ;
reg   [29-DSR-1: 0] kd_reg_r      ;
reg   [29-DSR  : 0] kd_reg_s      ;


always @(posedge clk_i) begin
   if (rstn_i == 1'b0) begin
      kd_reg   <= {29-DSR{1'b0}};
      kd_reg_r <= {29-DSR{1'b0}};
      kd_reg_s <= {29-DSR+1{1'b0}};
   end
   else begin
      kd_reg   <= kd_mult[29-1:DSR] ;
      kd_reg_r <= kd_reg;
      kd_reg_s <= $signed(kd_reg) - $signed(kd_reg_r);
   end
end

assign kd_mult = $signed(error) * $signed(set_kd_i) ;












//---------------------------------------------------------------------------------
//  Sum together - saturate output

wire  [   33-1: 0] pid_sum     ; // biggest posible bit-width
reg   [   14-1: 0] pid_out     ;

always @(posedge clk_i) begin
   if (rstn_i == 1'b0) begin
      pid_out    <= 14'b0 ;
   end
   else begin
      if ({pid_sum[33-1],|pid_sum[32-2:13]} == 2'b01) //positive overflow
         pid_out <= 14'h1FFF ;
      else if ({pid_sum[33-1],&pid_sum[33-2:13]} == 2'b10) //negative overflow
         pid_out <= 14'h2000 ;
      else
         pid_out <= pid_sum[14-1:0] ;
   end
end

assign pid_sum = $signed(kp_reg) + $signed(int_shr) + $signed(kd_reg_s) ;






assign dat_o = pid_out ;



endmodule

